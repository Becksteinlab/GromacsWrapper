...
This module implements ``test()`` function for gromacs modules.
...
import sys
import warnings
from gromacs.compat import string_types
...
def get_package_name(filepath):
    ...
    ----------
    filepath : str
        Path to a file. If the determination fails, "gromacs" is returned.

    Examples
    --------
    >>> gromacs.testing.nosetester.get_package_name('nonsense')
    'gromacs'
    ...
    # if package name determination failed, just default to gromacs
    if not pkg_name:
        return "gromacs"
...
class NoseTester(nosetester.NoseTester):
    ...
    This class is made available as gromacs.testing.nosetester.NoseTester, and
    ...
    gromacs, and ``()`` for released versions.  The purpose of this switching
    ...
    def _show_system_info(self):
        ...
        import gromacs
        print("gromacs version %s" % gromacs.version.get_version())

        gmxdir = os.path.dirname(gromacs.__file__)
        print("gromacs is installed in %s" % gmxdir)
    ...
    def test(self, label='fast', verbose=1, extra_argv=None,
        ...
        if raise_warnings is None:
            ...
            from gromacs import version
            from distutils.version import StrictVersion
            try:
                StrictVersion(version.get_version())
